...
class TestApiUtils(TestCase):
    ...
    def test_build_url(self):
        ...
        self.assertEqual(
            self.TEST_URL,
            api.build_url(
                "http://localhost:5678/", "api/v1/", "task", "random-task-1234"
            ),
        )
        self.assertEqual(
            self.TEST_URL,
    ...
    def test_match_regex(self):
        ...
        self.assertEqual(
            self.REGEXES[0], api.match_regex(self.REGEXES, "urn:ngsi-ld:Table:001")
        )
        self.assertEqual(
            self.REGEXES[1], api.match_regex(self.REGEXES, "urn:ngsi-ld:Machine:00001")
        )
    ...
    def test_parse_float(self):
        ...
        self.assertListEqual(
            [0.00001, 0.2, 11.4, 11], api.parse_float(["0.00001", "0.2", "11.4", "11"])
        )
        self.assertEqual(0.0002, api.parse_float("0.0002"))
        self.assertEqual(
            [0.00001, 0.2, 11.4, 11],
            api.parse_float(["0.00001", "0.2", "11.4", "11", "t"]),
        )
        self.assertEqual(
            [], api.parse_float(["0.00001t", "0.2e", "11.45h4", "1c1", "et"])
        )
